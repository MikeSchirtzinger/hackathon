...
function fail(message){cleanup();worker?.terminate();worker=null;queue?.close();queue=null;inflight=0;ready=false;finishing=false;pending=0;$('listen-zoom').disabled=false;status(message);}
function load(){
  return new Promise((resolve,reject)=>{
    ...
    worker.onmessage=async({data})=>{
      ...
      if(data.type==='final'){
        ...
        $('zoom-transcript').textContent=history;
        if(finishing&&!queue){finishing=false;$('listen-zoom').disabled=false;status('Stopped · transcript retained.');}
      }
    };
...
$('listen-zoom').onclick=async()=>{
  ...
  try{
    ...
    const result=await chrome.runtime.sendMessage({type:'capture-zoom',consumerTabId:tab.id});
