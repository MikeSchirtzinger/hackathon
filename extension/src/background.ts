...
import { base, type Meeting, type Note, type Settings, type TaskProposal } from './types';
...
chrome.alarms.onAlarm.addListener(alarm => {
  ...
  void serialize(async () => {
    ...
    try { await chrome.notifications.create(`meeting:${meeting.id}`, { type: 'basic', iconUrl: 'icon.png', title: meeting.title, message: 'Your saved meeting reminder is due. Click to join.', priority: 2 }); }
...
async function handle(message: Record<string, unknown>) {
  ...
  switch (message.type) {
    case 'state': {
      ...
      return { contexts, notes, tasks, meetings, absences, transcripts, outbox, settings: prefs, currentContextId: current?.value, lastStatus: last?.value, credentialsConfigured: !!auth.token, commands };
    }
    case 'capture': {
    ...
    case 'meeting-action': {
      ...
      if (action === 'listen') throw new Error('Transcription unavailable. The Nemotron audio transport is not connected.');
      ...
      if (action === 'join') { next.status = 'present'; await put('meetings', next); await chrome.tabs.create({ url: safeWebUrl(meeting.joinUrl) }); }
        ...
        // Speech remains unavailable until the transport implements immediate cancellation.
...
chrome.runtime.onMessage.addListener((message, sender, reply) => {
  if (!message || message.type === 'changed') return;
  if (sender.id !== chrome.runtime.id || !sender.url?.startsWith(chrome.runtime.getURL('panel.html'))) { reply({ ok: false, error: 'This operation requires the extension panel.' }); return; }
  if (message.type === 'settings' && message.syncMode === 'local') haltSync();
  const run = message.type === 'state' ? handle(message) : serialize(() => handle(message));
