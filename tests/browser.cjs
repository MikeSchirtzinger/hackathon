...
const ctx=await chromium.launchPersistentContext(require('node:fs').mkdtempSync(path.join(require('node:os').tmpdir(),'brevity-test-')),{channel:'chromium',headless:true,args:[`--disable-extensions-except=${extension}`,`--load-extension=${extension}`]});
try{
...
console.log(await p.evaluate(async()=>{
 ...
 // Real model: decode the known spoken sample in the existing worker.
 ...
 if(data.type==='final'){clearTimeout(timer);w.terminate();if(!/hackathon/i.test(data.text))reject(Error('unexpected transcript '+data.text));else resolve({queue:'PASS 40 seconds FIFO + drain',transcript:data.text});}
